...
@section('Reportes')
  ...
  <h1>Historial de atenciones de {!!$Involucrado_Reporte!!}</h1><br>
  ...
  <table border="1px" class="table table-bordered" id="Table_Atenciones_Cliente">
    <thead>
      <tr id="firstrow_reporte"><th>Nro</th><th>Id Cliente</th><th>Denominación</th><th>Atendido por:</th><th>Modalidad</th><th>Fecha</th><th>Problema</th><th>Solución Brindada</th><th>Tipo</th><th>Estado</th><th>Adjuntos</th></tr>
    </thead>
    <tbody>
        ...
        @for($i=0;$i<count($Datos_Reporte);$i++)
          ...
          <tr id="rows_reporte">
          <form method = "POST" action = {{action('AdminController@Descargar_Adjunto')}}>
          {{ csrf_field()}}
          <input type="hidden" name = "Id_Usuario" value = {!!$Datos_Usuario['ID_USUARIO']!!}></input>
          <td>{!!$i+1!!}</td>
